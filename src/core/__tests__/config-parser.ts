...
describe('Config parser', () => {
  beforeEach(() => {
    ...
    checkPathMock.mockImplementation(async () => Promise.resolve(true));
  });
  ...
  test('correctly validates existance of input and output paths', async () => {
    checkPathMock.mockImplementationOnce(async () => Promise.resolve(false));

    await testError({ inputDir: 'foo' }, 'inputDir', 'foo is not a directory');

    checkPathMock.mockImplementation(async val =>
      Promise.resolve(val !== 'bar')
    );
