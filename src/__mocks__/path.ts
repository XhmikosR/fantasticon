const _path = jest.requireActual('path');
const _relative = _path.relative;

const projectDir = _path.resolve(__dirname, '../../');

const resolve = (...paths: string[]) => {
  let path = '';

  for (const cur of paths) {
    path = path ? _path.join(path, normalise(cur)) : normalise(cur);
  }

  if (path.startsWith(projectDir)) {
    path = _path.join('/root/project', path.slice(projectDir.length));
  } else if (path.startsWith('/') && !path.startsWith('/root')) {
    path = _path.join('/root/', path.slice(1));
  } else if (path.startsWith('./')) {
    path = _path.join('/root/project/', path.slice(2));
  } else if (!path.startsWith('/')) {
    path = _path.join('/root/project/', path);
  }

  return normalise(path);
};

const relative = (a: string, b: string) =>
  normalise(_relative(_path.normalize(a), _path.normalize(b)));

const join = (...segments: string[]): string => {
  const trimmed: string[] = [];

  segments.forEach((current, i) => {
    const isFirst = i === 0;
    const isLast = i === segments.length - 1;

    if (!isFirst && current.startsWith('/')) {
      current = current.slice(1);
    }

    if (!isLast && current.endsWith('/')) {
      current = current.slice(0, Math.max(0, current.length - 1));
    }

    trimmed.push(current);
  });

  return trimmed.join('/');
};

const normalise = (path: string) => path.replace(/\\/g, '/');

const isAbsolute = (path: string) => path.startsWith('/root');

module.exports = { resolve, relative, join, isAbsolute };
